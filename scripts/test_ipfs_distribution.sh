...
#
# Server aliases:
#   fw16, local     - local machine
#   d, do           - DigitalOcean server
#   remote-nixos    - remote-nixos via jump host
#
# This script:
...
NIXPKGS_REF="${NIXPKGS_REF:-github:NixOS/nixpkgs/nixos-unstable}"  # Explicit nixpkgs for consistency

# SSH configuration for nested connections
REMOTE_NIXOS_JUMP="192.168.122.225"
REMOTE_NIXOS_HOST="alex@100.82.1.105"
REMOTE_NIXOS_SUDO_PASS="alex123"
...
run_on() {
    ...
    case "$server" in
        fw16|local)
            eval "$@"
            ;;
        remote-nixos|remote_nixos)
            ssh -J "$REMOTE_NIXOS_JUMP" "$REMOTE_NIXOS_HOST" "$@"
            ;;
        *)
            ssh "$server" "$@"
            ;;
...
sudo_on() {
    ...
    case "$server" in
        fw16|local)
            ...
            ;;
        remote-nixos|remote_nixos)
            ssh -J "$REMOTE_NIXOS_JUMP" "$REMOTE_NIXOS_HOST" "echo '$REMOTE_NIXOS_SUDO_PASS' | sudo -S $*"
            ;;
        *)
            ssh "$server" "sudo $*"
            ;;
...
STORE_PATH=$(run_on "$SERVER_A" "nix build ${NIXPKGS_REF}#$PACKAGE --no-link --print-out-paths 2>/dev/null" | head -1)
...
FETCH_OUTPUT=$(run_on "$SERVER_B" "NIX_DEBUG=1 nix build ${NIXPKGS_REF}#$PACKAGE --no-link --print-out-paths 2>&1" || true)
